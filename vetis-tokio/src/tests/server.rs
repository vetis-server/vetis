use crate::{
    http::Response,
    tests::{
        vetis_default_protocol, CA_CERT, IP6_SERVER_CERT, IP6_SERVER_KEY, SERVER_CERT, SERVER_KEY,
    },
...
async fn test_multiple_interfaces() -> Result<(), Box<dyn Error>> {
    ...
    let request = deboa::request::get("https://localhost:8080/hello")?
        .send_with(&client)
        .await?;
    ...
    let request = deboa::request::get(format!("https://{}:8081/hello", host))?
        .send_with(&client)
        .await?;
